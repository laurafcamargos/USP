...
__global__ void soma(int *A_dev_glb, int *B_dev_glb, int *C_dev_glb) {
    int i = blockDim.x * blockIdx.x + threadIdx.x; // linha
    int j = blockDim.y * blockIdx.y + threadIdx.y; // coluna
		// observe que a thread 1 do último bloco seria i = 3 * 5 + 1 = 16 (segfault)
    if (i < TAM && j < TAM)
        C_dev_glb[i * TAM + j] = A_dev_glb[i * TAM + j] + B_dev_glb[i * TAM + j];
} //locais ao kernel dessa função

int main(int argc, char **argv) {
    int *mA_hst, *mB_hst, *mC_hst; //declarando 3 matrizes A (host)
    int *mA_dev, *mB_dev, *mC_dev; //3 matrizes que vão parar na GPU
    ...
    // Define a quantidade de threads por bloco
		// no exemplo temos (3,3), ou seja, um bloco é constituído por uma matriz
	  // com 3 linhas com  3 threads por coluna
    dim3 threadsPerBlock(THREADS, THREADS); 

    // Define a quantidade de blocos por grade
    // no exemplo teremos (16 + 2) / 3 = 6, assim teremos grades (6,6)
    // cada grade então é composta por 6 linhas com 6 blocos por coluna 
